...
- Opening a supported audio file plays in the GUI chrome player (icon next to the project-sidebar Agents bell). Click the icon or playlist control to open a single global Player window (same pattern as Settings, not a workspace tab). Opening it again focuses that window; leftover Player tabs are closed. Open as text bypasses the player. Named playlists are global; opening a file appends to the selected playlist (Default is created on first file, and first Player open seeds bundled tone files if Default is empty). One engine; radio shares it. Playback stops when the GUI exits. Closing the Player window does not stop audio. The player is native egui, using the app theme and Lucide icons. Spectrum shows only while playing. Playlist footer: ADD/REM/SEL/MISC/LIST. Shuffle and repeat persist. Radio via ADD location / Stations.
